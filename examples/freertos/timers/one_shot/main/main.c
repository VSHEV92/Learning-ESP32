...
void app_main(void) {
    ...
    for (int i = 0; i < TIMER_NUMBER; i++) {
        // start timer
        xTimerStart(
            one_shot_timer[i],  // timer handler
            portMAX_DELAY       // max delay for waiting until command send to deamon command queue
        );
    }

    // wait until all timers are triggered and delete timers
